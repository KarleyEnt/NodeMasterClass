...
handlers._users.post = function (data, callback) {
  ...
  if (firstName && lastName && phone && password && tosAgreement) {
    // Make sure that the user doesn't already exists in the database
    _data.readM(setup.users, "phone", phone, function (err, data) {
      if (err || !data) {
        // Hash the password
        ...
        if (hashedPassword) {
          ...
          // Store the user
          _data.createM(setup.users, userObject, function(err) {
            if (!err) {
              callback(200);
            } else {
              callback(500, { Error: 'Could not store user data' });
            }
          });
        } else {
